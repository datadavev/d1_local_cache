...
Copied from: http://code.activestate.com/recipes/576918-python-short-url-generator/
'''
...
class ShortIdGenerator(object):
  ...
  def encode_id(self, n, min_length=MIN_LENGTH):
...
DEFAULT_ID_GENERATOR = ShortIdGenerator()

def encode(n):
    return DEFAULT_ID_GENERATOR.encode(n)

def decode(n):
    return DEFAULT_ID_GENERATOR.decode(n)

def enbase(n, min_length=MIN_LENGTH):
    return DEFAULT_ID_GENERATOR.enbase(n, min_length)

def debase(n):
    return DEFAULT_ID_GENERATOR.debase(n)

def encode_id(n, min_length=MIN_LENGTH):
    return DEFAULT_ID_GENERATOR.encode_id(n, min_length)

def decode_id(n):
    return DEFAULT_ID_GENERATOR.decode_id(n)
